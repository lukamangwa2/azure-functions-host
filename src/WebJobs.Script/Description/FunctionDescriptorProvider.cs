    ...
    public abstract class FunctionDescriptorProvider
    {
        ...
        public void VerifyResolvedBindings(FunctionMetadata functionMetadata, IEnumerable<FunctionBinding> inputBindings, IEnumerable<FunctionBinding> outputBindings)
        {
            ...
            if (unresolvedBindings.Any())
            {
                ...
                string errorMessage = Host.ExtensionBundleManager.IsExtensionBundleConfigured()
                    ? $"The binding type(s) '{allUnresolvedBindings}' were not found in the configured extension bundle. Please ensure the type is correct and the correct version of extension bundle is configured"
                    : $"The binding type(s) '{allUnresolvedBindings}' are not registered. Please ensure the type is correct and the binding extension is installed.";
        ...
        protected virtual ParameterDescriptor CreateTriggerParameter(BindingMetadata triggerMetadata, Type parameterType = null)
        {
            ...
            {
                string errorMessage = Host.ExtensionBundleManager.IsExtensionBundleConfigured()
                    ? $"The binding type '{triggerMetadata.Type}' were not found in the configured extension bundle. Please ensure the type is correct and the correct version of exension bundle is configured"
                    : $"The binding type '{triggerMetadata.Type}' is not registered. Please ensure the type is correct and the binding extension is installed.";
                throw new FunctionConfigurationException(errorMessage);
            }
